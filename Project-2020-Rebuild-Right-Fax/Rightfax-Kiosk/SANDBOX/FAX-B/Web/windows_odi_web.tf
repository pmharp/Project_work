resource "optumvm_windowsvm" "windows_odi_web" {
...
# ----------------------------------------------------------------------------------------------------------------------
  count = "${var.tf1211}${var.TFVAR_windows_web_count}"   
  # for_each = toset(["", ""])   
  #   # Enter server index name as quoted comma delimited values for state file
  #   # Example: ["WebOne", "WebTwo", "WebThree"] put 3 servers in statefile file indexed to WebOne, WebTwo and WebThree
# ---------------------------------------------------------------------------------------------------------------------- 

  # Edited by builder: Can mod for Day 2 Actions
  cpu               = "${var.tf1211}${var.TFVAR_windows_web_cpu}"       # Options are 1 - 16 using only EVEN numbers
  memory            = "${var.tf1211}${var.TFVAR_windows_web_memory}"    # Options are 4 - 128 using only EVEN numbers
  ...
  image            = "${var.tf1211}${var.TFVAR_windows_web_image}"    # options are 'v8-w2019', 'v8-w2016'
  ...
  reason              = "${var.tf1211}${var.TFVAR_windows_web_reason}"
...
output "windows_odi_web_server_info" {
  value = {
    for instance in optumvm_windowsvm.windows_odi_web:
