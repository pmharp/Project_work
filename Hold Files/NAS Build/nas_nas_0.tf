IACresource "optumnas_resourcenas" "nas_nas_0" {
  ...
  groups_read_only          = ["SED_All_Teams"]   # global group(s) list which can read share content, if filled in use ["value1","value2"]
  service_accounts_modify   = ["SED_All_Teams"]   # service account(s) list which can modify share content, if filled in use ["value1","value2"]
  service_accounts_readonly = ["SED_All_Teams"]   # service account(s) list which can read share content, if filled in use ["value1","value2"]
  ...
  requestor_msid          = "PHARP" # msid of the requestor
