$Credentials = Get-Credential
 ...
 #
 invoke-command -computername Wevet68399.ms.ds.uhc.com -ScriptBlock {get-disk | Where-partitionstyle -eq 'raw' | Initialize-Disk -PartitionStyle GPT -PassThru } -backend winrm -Credential $Credentials
 #  configures partition 2 as E drive
 invoke-command -computername Wevet68399.ms.ds.uhc.com -ScriptBlock {new-partition 2 -DriveLetter 'E' -UseMaximumSize | Format-Volume -FileSystem NTFS -NewFileSystemLabel 'e_app_1gb' -Confirm:$false } -backend winrm -Credential $Credentials
 pause
