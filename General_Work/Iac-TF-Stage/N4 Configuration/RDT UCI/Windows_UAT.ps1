$Credentials = Get-Credential
 ...
 #
 invoke-command -computername Apvep71770.ms.ds.uhc.com -ScriptBlock {get-disk | Where-Object partitionstyle -eq 'raw' | Initialize-Disk -PartitionStyle GPT -PassThru } -Credential $Credentials
 #  configures partition 2 as E drive
 invoke-command -computername Apvep71770.ms.ds.uhc.com -ScriptBlock {new-partition 2 -DriveLetter 'E' -UseMaximumSize | Format-Volume -FileSystem NTFS -NewFileSystemLabel 'App' -Confirm:$false } -backend winrm -Credential $Credentials
 pause
