resource "optumnas_resourcenas" "nas_nas_1" {
  ...
  request_title             = "Right Fax POC NAS"   # title of the request
  owner_msid                = "kprice26"   # MSID of the owner
  ask_app_name              = "UHGWM110-016291"   # name of the ask application
  admin_group               = "rightfax_ark"   # name of the admin group, 40 members or less
  backup_required           = "false"   # true or false
  storage                   = "200"   # size in GB
  ...
  share_name                = "fax_image"   # share name 
  ...
  groups_read_only          = ["rightfax_ark"]   # global group(s) list which can read share content, if filled in use ["value1","value2"]
  service_accounts_modify   = ["rightfax_ark"]   # service account(s) list which can modify share content, if filled in use ["value1","value2"]
  service_accounts_readonly = ["rightfax_ark"]   # service account(s) list which can read share content, if filled in use ["value1","value2"]
