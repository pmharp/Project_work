...
function RDTIN2{
    clear-host
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN2\RDT\BAIRDTIN.PS1 -Credential $Cred"
    Start_Menu($select)
}

function APPIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN2\APP\BAIAPPIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
...
function TaskIN2{
    clear-host
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN2\Task\BAITASKIN.PS1 -Credential $Cred"
    Start_Menu($select)
}

function WebIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN2\Web\BAIWebIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
function SQLIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN2\SQL-AO\BAISQLIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
...
function Start_Menu{
    Write-Host '
    *************************************************
    *   Right Fax BAI Build Menu For Prod IN2       *
    *************************************************
    *          Collective IN2                       *
    *               PROD                            *
