...
function RDTIN2{
    clear-host
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Stage\BAI-OUT1\RDT\BAISRDTIN.PS1 -Credential $Cred"
    Start_Menu($select)
}

function APPIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Stage\BAI-OUT1\APP\BAIAPPIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
...
function TaskIN2{
    clear-host
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Stage\BAI-OUT1\Task\BAITASKIN.PS1 -Credential $Cred"
    Start_Menu($select)
}

function WebIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Stage\BAI-OUT1\Web\BAIWebIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
function SQLIN2{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Stage\BAI-OUT1\SQL-AO\BAISQLIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
