Try {
  ...
  $chefwstitle = "Chef Workstation ($env:username)"

  if ( test-path $conemulocation )
  {
      start-process $conemulocation -argumentlist '/title',"`"$chefwstitle`"",'/cmd','powershell.exe','-noexit','-command',$chefwscommand
  }
  else
  {
      start-process powershell.exe -argumentlist '-noexit','-command',"$chefwscommand; (get-host).ui.rawui.windowtitle = '$chefwstitle'"
  }
}
Catch
{
