...
while( $addorchange -notlike "x"){  
$SeedServer = Read-Host -Prompt 'Type in the name of your Seed Server'
$RDPSet = Read-Host -Prompt 'How many RDP Tags per App Server'  
$addorchange = Read-Host -Prompt '  
To install a Collective , type collect  

To Install a single RightFax Server, type install  

To Quit press X  

'  

Write-Host 'Seed Server Set as:'  
...
if ($addorchange -like "install"){DisplayRDTtoServer($addorchange)}  
...
while( $addorchange1 -notlike "x"){  

  

$addorchange1 = Read-Host -Prompt '  

  

To Install Remote DocTransport Server(s), type rdt 

To Skip Installing Remote DocTransports, type x 
...
if ($addorchange1 -like "rdt"){PrepServer($addorchange1);Restart-Computer -ComputerName $Server -Wait -Delay 2 -force}  
...
while( $addorchange2 -notlike "x"){  

  

$addorchange2 = Read-Host -Prompt '  
...
if ($addorchange2 -like "rdt"){PrepServer($addorchange2);Restart-Computer -ComputerName $Server -Wait -Delay 2 -force}  
...
Write-Host -ForegroundColor Green "Start File Copy Funtion  Line 448" $S 

        #StartFileCopy 

        #start-sleep -Seconds 30 

Write-Host -ForegroundColor Green "StartInstallPrepFuntion  Line 448" $S 

        #StartInstallPrep 

        #start-sleep -Seconds 30 

Write-Host -ForegroundColor Green "StartInstallFunction Line 465" $S 

        #StartInstall 

        #start-sleep -Seconds 30 

Write-Host -ForegroundColor Green "StartInstallCleanUpFunction  Line 468" $S 

        #StartInstallCleanup 

Write-Host -ForegroundColor Green "the end  Line 496" $S 