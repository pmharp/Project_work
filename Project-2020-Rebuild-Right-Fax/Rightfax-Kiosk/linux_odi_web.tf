resource "optumvm_linuxvm" "linux_odi_web" {
...
# ----------------------------------------------------------------------------------------------------------------------
  count = "${var.tf1211}${var.TFVAR_linux_web_count}"   
  # for_each = toset(["", ""])   
...
# ----------------------------------------------------------------------------------------------------------------------
  ...
  cpu               = "${var.tf1211}${var.TFVAR_linux_web_cpu}"         # Options are 1 - 16 using only EVEN numbers
  memory            = "${var.tf1211}${var.TFVAR_linux_web_memory}"      # Options are 4 - 128 using only EVEN numbers
  ...
  image                = "${var.tf1211}${var.TFVAR_linux_web_image}"           # options are 'v8-rhel-8', 'v8-rhel-7', 'v8-centos-7'
  ...
  reason              = "${var.tf1211}${var.TFVAR_linux_web_reason}"
...
output "linux_odi_web_server_info" {
  value = {
    for instance in optumvm_linuxvm.linux_odi_web:
