# resource "optumvm_wildfly_odi" "linux_odi_app_wildfly" {
...
# # ----------------------------------------------------------------------------------------------------------------------
#   count = "${var.tf1211}${var.TFVAR_linux_wildfly_count}"   
#   # for_each = toset(["", ""])   
#   #   # Enter server index name as quoted comma delimited values for state file
#   #   # Example: ["wildfly1", "wildfly2", "myApp1"] put 3 servers in statefile file indexed to wildfly1 wildfly2 and myApp1
# # ----------------------------------------------------------------------------------------------------------------------

# # Edited by builder: Can mod for Day 2 Actions
#   cpu               = "${var.tf1211}${var.TFVAR_linux_wildfly_cpu}"     # Options are 1 - 16 using only EVEN numbers
#   memory            = "${var.tf1211}${var.TFVAR_linux_wildfly_memory}"  # Options are 4 - 128 using only EVEN numbers
...
#   image                = "${var.tf1211}${var.TFVAR_linux_wildfly_image}"       # options are 'v8-rhel-8', 'v8-rhel-7', 'v8-centos-7'
#   lobname              = "${var.tf1211}${var.TFVAR_linux_wildfly_lobname}"     # options 'LOB Internal DO_NOT_USE'
#   wildflyversion       = "${var.tf1211}${var.TFVAR_linux_wildfly_version}"     # options are '20.0.0'
...
#   reason              = "${var.tf1211}${var.TFVAR_linux_wildfly_reason}"
#   vraenv              = "${var.tf1211}${upper(var.TFVAR_odi_vraenv)}"
# }

# output "linux_odi_app_wildfly_server_info" {
#   value = {
#     for instance in optumvm_wildfly_odi.linux_odi_app_wildfly:
