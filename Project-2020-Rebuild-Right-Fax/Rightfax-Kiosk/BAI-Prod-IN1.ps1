...
function RDTIN1{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN1\RDT\BAIRDTIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}

function APPIN1{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN1\APP\BAIAPPIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
...
function TaskIN1{
    clear-host
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN1\Task\BAITaskIN.PS1 -Credential $Cred"
    Start_Menu($select)
}

function WebIN1{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN1\Web\BAIWebIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
function SQLIN1{
    Invoke-expression -Command "E:\SC\Rightfax-Kiosk\Prod\BAI-IN1\SQL-AO\BAISQLIN.PS1 -Credential $Cred"
    clear-host
    Start_Menu($select)
}
...
function Start_Menu{
    Write-Host '
    *************************************************
    *   Right Fax BAI Build Menu For Prod IN1       *
    *************************************************
    *          Collective IN1                       *
    *               PROD                            *
