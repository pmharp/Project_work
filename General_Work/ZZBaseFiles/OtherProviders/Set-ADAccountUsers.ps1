...
.NOTES   
Name: Set-ADAccountUsers.ps1
Author: Patrick Harp

.EXAMPLE   
.\Set-ADAccountUsers.ps1.ps1 -Computer Server01 -Trustee test

Description:
Will set the the test account as a Local Administrator on Server01

.EXAMPLE   
.\Set-ADAccountUsers.ps1.ps1 -Computer 'Server01,Server02' -Trustee Contoso\HRTest

Description:
Will set the HRTest group in the contoso domain as Local Administrators on Server01 and Server02

.EXAMPLE   
.\Set-ADAccountUsers.ps1 -InputFile C:\ListofComputers.txt -Trustee User01
...
if (!$InputFile) {
	...
	$Computer | ForEach-Object {
		$_
		Write-Host "Adding `'$ADResolved`' to Users group on `'$_`'"
		try {
			([ADSI]"WinNT://$_/Users,group").add($Trustee)
