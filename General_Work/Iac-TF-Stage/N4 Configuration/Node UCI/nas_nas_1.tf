resource "optumnas_resourcenas" "nas_nas_1" {
  ...
  request_title             = "Right Fax 2020 POC NAS"   # title of the request
  owner_msid                = "kprice26"   # MSID of the owner
  ask_app_name              = "Enterprise Fax - Business Application Integrations"   # name of the ask application
  admin_group               = "ms\rightfax"   # name of the admin group, 40 members or less
  ...
  groups_modify             = ["ms\rightfax"]   # global group(s) list which can modify share content, if filled in use ["value1","value2"]
  groups_read_only          = ["ms\rightfax"]   # global group(s) list which can read share content, if filled in use ["value1","value2"]
