...
# Designed to 
# .  ./util_getlivevmspecs.ps1 -kb                   keyboard input results sent to screen
# .  ./util_getlivevmspecs.ps1 -kb -log              Logging and keyboard input results sent to screen
# .  ./util_getlivevmspecs.ps1 -list -screen         Pull servers from ServerList.txt, results sent to screen
# .  ./util_getlivevmspecs.ps1 -list -screen -log    Logging and Pull servers from ServerList.txt, results sent to screen
# .  ./util_getlivevmspecs.ps1 -list -file           Pull servers from ServerList.txt, results sent getlivevmspecs_.....txt
# .  ./util_getlivevmspecs.ps1 -list -file  -log     Logging and Pull servers from ServerList.txt, results sent getlivevmspecs_.....txt
...
function getlivevmspecs {
    ...
    $request = Invoke-RestMethod -Method 'Post' -Uri $apiHost/api/vm/v8/getlivevmspecs/?sync=true -Body $requestBody -Headers @{"Content-Type"="$contentType"; "Authorization"="Bearer $apiToken"}
    $uuid = $request.uuid
    do{
...
if ( $kb ) { 
    ...
    getlivevmspecs -servername $servername -OutputType "Screen" -OutputFileName ""
}
elseif ( ($list -and ($screen -or $file)) ) {
    ...
    $OutputFileName = "LogFiles/getlivevmspecs_${RunDateTime}.txt"
    ...
    if ($file) {
        ...
        $headerText = "Beginning getlivevmspecs Run on " +  $RunDateTime
    ...
    if (Test-Path $InputFileName)  { 
        ...
        foreach ($svr in $tmpContentArray) {
            if ($screen) {  getlivevmspecs -servername $svr -OutputType "Screen" -OutputFileName "" }
            elseif ($file) {  getlivevmspecs -servername $svr -OutputType "File" -OutputFileName $OutputFileName }
    ...
    echo "  ./util_getlivevmspecs.ps1 -kb                   keyboard input results sent to screen"
    echo "  ./util_getlivevmspecs.ps1 -kb -log              Logging and keyboard input results sent to screen"
    echo "  ./util_getlivevmspecs.ps1 -list -screen         Pull servers from ServerList.txt, results sent to screen"
    echo "  ./util_getlivevmspecs.ps1 -list -screen -log    Logging and Pull servers from ServerList.txt, results sent to screen"
    echo "  ./util_getlivevmspecs.ps1 -list -file           Pull servers from ServerList.txt, results sent SvrDetail.txt"
    echo "  ./util_getlivevmspecs.ps1 -list -file  -log     Logging and Pull servers from ServerList.txt, results sent SvrDetail.txt"
    echo "--------------------------------------------------------------------------------------`r`n" 
}
# End Program Logic-----------------------------------