resource "optumnetwork_ltm" "net_ltm_only_1" {
  ...
  device_names     = ["Wevet68399"]        # ShortName of the target devices.  EX:  ["servernameA","ServernameB"]
  fqdn             = "IAC_MTNW"          # FQDN of the LTM
  data_center      = "ELR"       # Options are 'CTC' or 'ELR' where servers and the ltm are located
  line_of_business = "OURS" # LOB
  is_public        = "FALSE"     # TRUE=DMZ Presentation zone devices, FALSE when devices in Intranet or DMZ-App, DB or Tools zones,

  ownership_details = {
    adminGroup = "UHTSE_Viewer" # Global Group - CASE matters need to be less than 40 members
    msid       = "PHARP" # MSID of the owner
  }
