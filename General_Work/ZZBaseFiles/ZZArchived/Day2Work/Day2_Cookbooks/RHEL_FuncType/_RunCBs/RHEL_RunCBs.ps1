...
function ProcessRunlistOnServer {
    ...
    # Original:       $Cmd _PSCP = "pscp  -l ${UserID} -pw ${UserPWD} -r cookbooks ${fnsSvrName}.uhc.com:/home/${UserID}"
    # Pre 07/03/2020: $Cmd_PSCP = "pscp  -l ${UserID} -r cookbooks ${fnsSvrName}.uhc.com:/home/${UserID}"
    $Cmd_PSCP = "pscp -scp -r cookbooks  ${UserID}@${fnsSvrName}.uhc.com:/home/${UserID}"
    set-location "../"
    ...
    if ( $LastExitCode -eq 0) { 
        ...
        foreach ($cookbook in $fnsCBArray) {
            # Pre 07/03/2020: $Cmd_SSH = "ssh -oStrictHostKeyChecking=no ${UserID}@${fnsSvrName}.uhc.com sudo chef-client -zo ${cookbook}"
